...
function Sidebar({ onNavigate }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-5 py-5 border-b border-ink-700/70"><Brand /></div>
      <nav className="flex-1 overflow-y-auto scrollbar-thin px-3 py-4 space-y-1">
        {NAV.filter((n) => !n.admin || isAdmin).map(({ to, label, icon: Icon, id }) => (
          <NavLink key={to} to={to} onClick={onNavigate} data-testid={`internal-sidebar-nav-${id}`}
            className={({ isActive }) => `group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition-colors duration-150 ${isActive ? "bg-amber-brand text-ink-900" : "text-slate-300 hover:bg-ink-700/60 hover:text-white"}`}>
      ...
      <div className="border-t border-ink-700/70 p-4">
        <div className="flex items-center gap-3">
          ...
          <button data-testid="sidebar-logout-button" title="Keluar" onClick={async () => { await logout(); navigate("/", { replace: true }); }} className="rounded-lg p-2 text-slate-400 hover:text-red-400 hover:bg-ink-700/60 transition-colors"><LogOut size={16} /></button>
        </div>
      </div>
    </div>
  );
...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-ink-900 text-slate-200 grain relative">
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-72 flex-col border-r border-ink-700/70 bg-ink-950/80 backdrop-blur-md z-30"><Sidebar /></aside>
      {open && (
        <div className="lg:hidden fixed inset-0 z-40">
          <div className="absolute inset-0 bg-black/60" onClick={() => setOpen(false)} />
          <aside className="absolute inset-y-0 left-0 w-72 bg-ink-950 border-r border-ink-700 animate-rise"><Sidebar onNavigate={() => setOpen(false)} />
      ...
      <div className="lg:pl-72">
        <header className="lg:hidden sticky top-0 z-20 flex items-center justify-between border-b border-ink-700/70 bg-ink-950/80 backdrop-blur-md px-4 py-3">
          <Brand compact />
          <button onClick={() => setOpen(true)} className="rounded-lg p-2 text-slate-300 hover:bg-ink-700/60" data-testid="sidebar-open-button"><Menu size={20} /></button>
        </header>
